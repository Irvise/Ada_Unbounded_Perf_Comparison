...
procedure A033919_PragmARC
  with SPARK_Mode => On
is

   function Is_Prime (N : Unbounded_Integer) return Boolean
     with Inline
   is
      --  Make the Big_Temp the input and then overwrite to see if there is only one allocation that takes place
      Big_Temp : Unbounded_Integer := N; -- It does not seem to be helping... Maybe when we do Big_Temp := 7 below GNAT cleans the memory...
      Counter : Long_Long_Long_Integer := 1;
      Big_2 : constant Unbounded_Integer := To_Unbounded_Integer (2);
      Big_3 : constant Unbounded_Integer := To_Unbounded_Integer (3);
      Big_5 : constant Unbounded_Integer := To_Unbounded_Integer (5);
      Big_0 : constant Unbounded_Integer := To_Unbounded_Integer (0);
   begin
      ...
      Big_Temp := To_Unbounded_Integer (7);
   ...
   subtype Big_Prime is Unbounded_Integer
   ...
   type Powers_of_Two_Table is array (Positive'First .. Upper_Table_Bound) of Unbounded_Integer -- Changed based index from 0 to 1, that simplifies things
     with Dynamic_Predicate => (for all I in Powers_Of_Two_Table'Range => Powers_Of_Two_Table (I) = To_Unbounded_Integer (2)**I);

   Powers_of_Two : constant Powers_of_Two_Table := [for I in Powers_of_Two_Table'Range => To_Unbounded_Integer (2)**I]; -- We initialise the table and make it constant
   ...
   begin
      while M < K and then M < Powers_of_Two_Table'Last loop
         N := Powers_of_Two (M) + To_Unbounded_Integer (K);
         pragma Debug (Ada.Text_IO.Put_Line ("Iteration " & M'Image & " for number " & K'Image));
         pragma Debug (Ada.Text_IO.Put_Line ("DEBUG: (function Is_A033919) : N is " & N'Image));
         if Is_Prime (N) then
            return False;
         end if;
         M := M + 1;
